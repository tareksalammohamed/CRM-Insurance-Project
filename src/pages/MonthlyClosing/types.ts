...
export interface SupervisorAgg {
  ...
  // ── حقول مذكرة التوصية التلقائية بصرف "فرق التنسيب" (راجع
  // RecommendationMemo.tsx) — بتتحسب دايمًا (تكلفتها محدودة)، لكن بتتستخدم
  // بس لو الشرطين اتحققوا: عدد رؤساء المجموعات الفعلي = 3، ونسبة التحقيق
  // ≥ 151%. لا تأثير لها على أي حساب أو عرض موجود قبل كده.
  /** الهدف الشهري الثابت لصاحب الصف (users.target)، أو 0 لو مش متحدد. */
  target: number;
  /** نسبة التحقيق = round(total / target × 100)، أو 0 لو مفيش هدف محدد. */
  achievementRate: number;
  /** عدد رؤساء المجموعات التابعين مباشرة (هيكليًا، بغض النظر عن وجود حركة
   * فعلية الشهر ده من عدمه) — لفحص شرط "3 رؤساء مجموعات" بدقة. */
  directGroupLeaderCount: number;
  /** أقرب مدير فى التسلسل الإداري درجته "مراقب عام" (لو موجود فى نطاق
   * البيانات المجلوبة لهذا التقرير) — يُستخدم لتوقيع مذكرة التوصية. */
...
export interface BasicUser {
  ...
  role: UserRole;
  manager_id: string | null;
  /** الهدف الشهري الثابت لهذا المستخدم (نفس عمود users.target المستخدم فى
   * لوحة التحكم لحساب نسبة التحقيق) — يُستخدم هنا فقط لمذكرة التوصية
   * التلقائية بصرف فرق التنسيب (راجع RecommendationMemo). */
  target?: number | null;
