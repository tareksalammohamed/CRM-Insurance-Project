-- حماية إضافية ضد تكرار العميل فى الاستيراد لما مفيش رقم قومي فى الصف —
-- بس مقصورة على وثائق "حماية واستثمار" فقط (زي ما طلب طارق بالظبط)، لأن
-- دي الحالة الوحيدة اللي فعلاً بيحتاج فيها نفس العميل أكتر من صف مستورد.
-- لأي نوع وثيقة تاني، السلوك القديم فاضل زي ما هو (عميل جديد دايماً لما
-- مفيش رقم قومي) — مطابقة بالاسم لوحده أخطر من كده لو معندناش سبب واضح
-- ليها.
--
-- المطابقة: نفس الاسم (بعد إزالة المسافات الزايدة وتوحيد الحالة) + نفس
-- الوكيل + مفيش رقم قومي مسجّل على العميل الموجود برضه (لو عنده رقم قومي
-- فعلي، يبقى مش نفس الحالة اللي بنتكلم عنها، ومنعملش مطابقة بالاسم عليه).
...
BEGIN
    ...
    ELSIF p_policy_type = 'protection_investment' THEN
        -- مفيش رقم قومي، لكن وثيقة "حماية واستثمار" هي الحالة الوحيدة اللي
        -- محتاجة صفوف متعددة لنفس العميل الحقيقي — فبندوّر بالاسم + الوكيل
        -- على عميل موجود ملوش رقم قومي أصلاً (يعني نفس ظرف عدم توفر الرقم
        -- القومي)، ونستخدمه بدل ما ننشئ نسخة تانية.
        --
        -- مفيش قيد UNIQUE على الاسم زي الرقم القومي، فمنقدرش نستخدم ON
        -- CONFLICT هنا. بدالها: قفل استشاري (advisory lock) على مفتاح
        -- (الوكيل + الاسم) يمنع صفين متوازيين لنفس العميل الجديد (الاستيراد
        -- شغال بعدة نداءات متوازية) من عمل SELECT فاضي فى نفس اللحظة
        -- وإنشاء نسختين — القفل بيتحرر تلقائياً آخر المعاملة.
        PERFORM pg_advisory_xact_lock(hashtextextended(v_agent_id::text || ':' || btrim(lower(p_customer_name)), 0));

        SELECT id INTO v_customer_id
        FROM customers
        WHERE btrim(lower(name)) = btrim(lower(p_customer_name))
    ...
    ELSE
        -- أي نوع وثيقة تاني غير "حماية واستثمار": نفس السلوك القديم —
        -- عميل جديد دايماً لما مفيش رقم قومي (مطابقة بالاسم لوحدها مخاطرة
        -- مش مبررة هنا)
        BEGIN
            INSERT INTO customers (
                name, national_id, phone, address, birth_date, occupation, marital_status, owner_id, is_imported
